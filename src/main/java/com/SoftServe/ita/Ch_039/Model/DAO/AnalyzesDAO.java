package com.SoftServe.ita.Ch_039.Model.DAO;
...
public class AnalyzesDAO {
    ...
    public synchronized boolean addAnalysis(Analysis analysis, Patient patient) throws PersistenceException {
        try {
  ...
  /*  public Analysis getAnalyzesById(long id) throws PersistenceException {
        ...
        try {
            manager.getTransaction().begin();
            analysis = manager.getReference(Analysis.class, id);
            manager.getTransaction().commit();
        } catch (PersistenceException e) {
